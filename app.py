...
def app_data_dir() -> Path:
    root = Path(os.getenv("LOCALAPPDATA", Path.home())) / "SamirSoft"
...
class Database:
    def __init__(self) -> None:
        self.path = app_data_dir() / "samir_soft.db"
...
class App(tk.Tk):
    def __init__(self) -> None:
        ...
        self.db = Database()
        self.user = None
        self.title(f"{APP_NAME} • CBDS Business Suite")
    ...
    def export_xlsx(self):
        ...
        try:
            ...
            ws.merge_cells("A2:J2");ws["A2"]=f"Official CBDS report • Generated by Samir Soft {APP_VERSION} • {datetime.now():%Y-%m-%d %H:%M}";ws["A2"].font=Font(italic=True,color="666666");ws["A2"].alignment=Alignment(horizontal="center")
    ...
    def export_pdf(self):
        ...
        try:
            ...
            styles=getSampleStyleSheet();story=[PDFImage(str(resource_path("assets/cbds_logo.png")),width=60*mm,height=30*mm),Paragraph("<b>Product Profitability Report</b>",styles["Title"]),Paragraph(f"Generated by Samir Soft {APP_VERSION} on {datetime.now():%Y-%m-%d %H:%M}",styles["Normal"]),Spacer(1,5*mm)]
