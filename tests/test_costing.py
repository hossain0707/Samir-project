...
class CostingTests(unittest.TestCase):
    ...
    def test_workbook_formula_equivalence(self):
        product = Product(purchase_rmb=39, weight_g=380, shipping_rate_bdt=780)
        result = calculate_cost(product, total_purchase_rmb=1000, sourcing_rmb=277.56, rate=19.2, retail=30, wholesale=20)
        self.assertAlmostEqual(result.shipping_bdt, 296.4)
        self.assertAlmostEqual(result.sourcing_rmb, 10.82484)
        self.assertAlmostEqual(result.subtotal_rmb, 49.82484)
        self.assertAlmostEqual(result.total_bdt, 1253.036928)
    ...
    def test_zero_total_purchase_is_safe(self):
        product = Product(purchase_rmb=0, weight_g=0, shipping_rate_bdt=0)
